...
class MTCbmForexHistory(models.TransientModel):
    ...
    def get_latest_rate(self):
        ...
        updated_date = time.strftime(
            '%Y-%m-%d', time.localtime(int(response.get('timestamp'))))
        ...
        for currency, rate in rates.items():
            if currency in list_active_currencies_name:
                if self.env.user.company_id.currency_id.name not in list(
                        rates.keys()):
                    rate = 1 / float(rate.replace(',', ''))
                else:
                    rate = float(rate.replace(',', ''))
                currency_id = active_currencies.search([('name', '=', currency)
                                                        ]).id
